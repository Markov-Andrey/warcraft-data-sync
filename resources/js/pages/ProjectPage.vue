<template>
    <div class="p-6">
        <h1 class="text-2xl font-bold text-yellow-400 mb-6">WarCraft Data Sync</h1>

        <!-- Loading -->
        <div v-if="pageLoading" class="text-gray-400 text-sm">Loading...</div>
        ...
        <template v-else>
            ...
            <div class="flex gap-3 mb-4">
                ...
                >
                    {{ actionLoading === 'sync' ? '⏳ Syncing...' : '🔄 Sync to Child' }}
                </button>
                <button
                    @click="compileBuild"
                    :disabled="!!actionLoading"
                ...
                >
                    {{ actionLoading === 'build' ? '⏳ Building...' : '🛠 Compile Build' }}
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-gray-800 rounded-lg p-4 border border-gray-700">
                    ...
                    <div class="space-y-1">
                        <div v-for="file in copyFiles" :key="file" class="text-xs text-gray-300">📄 {{ file }}</div>
                ...
                <div class="bg-gray-800 rounded-lg p-4 border border-gray-700">
                    ...
                    <div class="space-y-1">
                        <div v-for="file in exceptionsFiles" :key="file" class="text-xs text-gray-300">❌ {{ file }}</div>
...
onMounted(async () => {
    try {
        ...
        childProjects.value = data.childProjects;
        copyFiles.value = data.copyFiles;
        exceptionsFiles.value = data.exceptionsFiles;
        currentProject.value = data.configInfo.current_project ?? '';
        buildVersion.value = data.configInfo.build_version ?? '';
    } catch {
        showNotification('error', 'Failed to load project info');
    } finally {
...
async function syncToChild() {
    ...
    try {
        const { data } = await axios.get('/copy-child');
        if (data.success) {
...
async function compileBuild() {
    ...
    try {
        const { data } = await axios.get('/set-build');
        if (data.success) {
...
async function switchProject() {
    try {
        const { data } = await axios.post('/switch-project', { select: currentProject.value });
        if (data.success) {
...
async function updateVersion() {
    try {
        const { data } = await axios.post('/update-version', { version: buildVersion.value });
        if (data.success) {
